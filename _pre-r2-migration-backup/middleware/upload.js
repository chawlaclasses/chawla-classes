...
// ── uploadNote ────────────────────────────────────────────────────────────────
// FIX (production hardening, Phase 3, issue #4): multer's diskStorage does
// NOT create a missing destination directory — it fails uploads with
// ENOENT instead (verified directly; this isn't documented prominently).
// Every other uploader in this file already guarded against that except
// these two. On a fresh checkout/deploy where notes/ and uploads/ don't
// already exist on disk (now more likely than before: see
// PERSISTENT_ROOT_DIR in config/index.js — a brand-new persistent disk
// mount starts genuinely empty), the very first note or PDF upload would
// have failed until someone manually created the folder.
